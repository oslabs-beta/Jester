...
// This component will display code snippets from the database if a user is logged in, or from state if a user is not logged in
...
const ClipBoard = () => {
  ...
  const handleClear = () => { 
    if (isLoggedIn) { // delete the project from the database if user logged in
      dispatch(deleteProject(projectId));
      navigate('/');
    } else { // delete clipboard data from state if user not logged in
      dispatch(clearClipboardState());
    }
  };

  useEffect(() => {
    if (isLoggedIn) { // fetch code snippets from db if user logged in
